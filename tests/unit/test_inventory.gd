...
func test_empty_inventory_has_zero_items() -> void:
	var inv: Inventory = Inventory.new(10)
	assert_eq(inv.item_count(), 0)

func test_add_item_increases_count() -> void:
	var inv: Inventory = Inventory.new(10)
	var item: InventoryItem = InventoryItem.new("Widget", 1)
	inv.add(item)
...
func test_remove_nonexistent_returns_false() -> void:
	var inv: Inventory = Inventory.new(10)
	var item: InventoryItem = InventoryItem.new("Ghost", 1)
...
func test_over_capacity_when_exceeded() -> void:
	var inv: Inventory = Inventory.new(3)
	inv.add(InventoryItem.new("A", 2))
...
func test_send_to_top_moves_item_first() -> void:
	var inv: Inventory = Inventory.new(10)
	var a: InventoryItem = InventoryItem.new("A", 1)
	var b: InventoryItem = InventoryItem.new("B", 1)
	var c: InventoryItem = InventoryItem.new("C", 1)
...
func test_changed_emitted_on_remove() -> void:
	var inv: Inventory = Inventory.new(10)
	var item: InventoryItem = InventoryItem.new("X", 1)
	inv.add(item)
	watch_signals(inv)
	inv.remove(item)
	assert_signal_emitted(inv, "changed")
