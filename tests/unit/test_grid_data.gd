...
func test_place_single_cell_returns_valid_id() -> void:
	var shape := PieceShape.new()
	var id := grid.place_piece(shape, 1, 1)
	assert_gt(id, 0)

func test_placed_cell_shows_piece_id() -> void:
	var shape := PieceShape.new()
	var id := grid.place_piece(shape, 2, 3)
...
func test_place_l_shape_occupies_all_cells() -> void:
	...
	var id := grid.place_piece(shape, 1, 2)
	assert_gt(id, 0)
...
func test_get_piece_info_returns_correct_data() -> void:
	var shape := PieceShape.new()
	var id := grid.place_piece(shape, 3, 5)
	var info := grid.get_piece_info(id)
...
func test_ids_are_unique() -> void:
	var shape := PieceShape.new()
	var id1 := grid.place_piece(shape, 1, 1)
	var id2 := grid.place_piece(shape, 1, 3)
	assert_ne(id1, id2)
